...
// | Original Author:  Alan Knowles <alan@akbkhome>                               |
// +----------------------------------------------------------------------+
//
// $Id: $
//
...
//  The parent.
//
$GLOBALS['_FLY_FLEXY_TOKEN']['base'] = 0;
$GLOBALS['_FLY_FLEXY_TOKEN']['state'] = 0;
$GLOBALS['_FLY_FLEXY_TOKEN']['statevars'] = array();
$GLOBALS['_FLY_FLEXY_TOKEN']['activeForm'] = '';
$GLOBALS['_FLY_FLEXY_TOKEN']['tokens'] = array();
$GLOBALS['_FLY_FLEXY_TOKEN']['gettextStrings'] = array();
$GLOBALS['_FLY_FLEXY_TOKEN']['activeFormId'] = 0;
$GLOBALS['_FLY_FLEXY_TOKEN']['flexyIgnore'] = false;
/**
...
class Fly_Flexy_Token {
    ...
    */
    ...
    function factory($token, $value, $line, $charPos=0) {
        ...
        if (! isset($loaded[$token])) {
            if (class_exists(__CLASS__ . '_' . $token)) {
                $loaded[$token] = true;
            } else {
                $file = dirname(__FILE__) . "/Token/" . basename($token . ".php");
                if (file_exists($file)) {
                    require_once $file;
                }
                if (class_exists(__CLASS__ . '_' . $token)) {
    ...
    */
    function compileChildren( &$compiler) {
        ...
        foreach ($this->children as $child) {
            ...
            if (method_exists($child, 'compile')) {
    ...
    */
    ...
    function buildTokens($tokenizer)
    {
        ...
        // reset my globals..
        $_FLY_FLEXY_TOKEN['base'] = 0;

        $_FLY_FLEXY_TOKEN['statevars'] = array();
        $_FLY_FLEXY_TOKEN['state'] = 0;

        $_FLY_FLEXY_TOKEN['flexyIgnore'] = false;
        if (@$GLOBALS['_FLY_FLEXY']['currentOptions']['flexyIgnore']) {
            $_FLY_FLEXY_TOKEN['flexyIgnore'] = true;
        }
        $_FLY_FLEXY_TOKEN['activeFormId'] = 0;
        $_FLY_FLEXY_TOKEN['activeForm'] = '';

        $_FLY_FLEXY_TOKEN['tokens'] = array(new Fly_Flexy_Token);
        $_FLY_FLEXY_TOKEN['tokens'][0]->id =0;
        $_FLY_FLEXY_TOKEN['gettextStrings'] = array();
        $i=1;
        ...
        while ($t = $tokenizer->yylex()) {
            ...
            if ($t == FLY_FLEXY_TOKEN_ERROR) {
                ...
                return Fly_Flexy::raiseError('Fly_Flexy::Syntax error in ".
            ...
            if ( is_object($tokenizer->value) && $tokenizer->value->token == 'Php' ) {
                echo $tokenizer->yytext();echo $tokenizer->yyline;exit();
                if (!$GLOBALS['_FLY_FLEXY']['currentOptions']['allowPHP']) {
                    return Fly_Flexy::raiseError('PHP code found in script (Token)',
                        FLY_FLEXY_ERROR_SYNTAX,FLY_FLEXY_ERROR_RETURN
                    );
                }

                if ($GLOBALS['_FLY_FLEXY']['currentOptions']['allowPHP'] === 'delete') {
                    continue;
                }

            }
            $i++;
            $_FLY_FLEXY_TOKEN['tokens'][$i] = $tokenizer->value;
            $_FLY_FLEXY_TOKEN['tokens'][$i]->id = $i;
            ...
            if (isset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXYSTART'])) {
                unset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXYSTART']);
                $_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXY:START'] = true;
            }

            if (isset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXYSTARTCHILDREN'])) {
                unset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXYSTARTCHILDREN']);
                $_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXY:STARTCHILDREN'] = true;
            }

            if (isset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXY:START'])) {
                $_FLY_FLEXY_TOKEN['base'] = $i;
                unset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXY:START']);
            }

            if (isset($_FLY_FLEXY_TOKEN['tokens'][$i]->ucAttributes['FLEXY:STARTCHILDREN'])) {
                $_FLY_FLEXY_TOKEN['base'] = $i;
            }
        ...
        $i = $total;
        $_FLY_FLEXY_TOKEN['tokens'][$i] = new Fly_Flexy_Token;
        $_FLY_FLEXY_TOKEN['tokens'][$i]->id = $total;
        $_FLY_FLEXY_TOKEN['tokens'][0]->close = &$_FLY_FLEXY_TOKEN['tokens'][$total];

        // now is it possible to connect children...
        // now we need to GLOBALIZE!! -


        $_FLY_FLEXY_TOKEN['tokens'] = $res;

        Fly_Flexy_Token::buildChildren(0);
        //new Gtk_VarDump($_FLY_FLEXY_TOKEN['tokens'][0]);
        //echo '<PRE>';print_R($_FLY_FLEXY_TOKEN['tokens'][$_FLY_FLEXY_TOKEN['base']]  );
        return $_FLY_FLEXY_TOKEN['tokens'][$_FLY_FLEXY_TOKEN['base']];
    }
    /**
    ...
    */
    function buildChildren($id)
    {
        ...
        for ($i=$start; $i<$end; $i++) {
            ...
            if (isset($_FLY_FLEXY_TOKEN['tokens'][$i]) &&
                is_object($_FLY_FLEXY_TOKEN['tokens'][$i]->close)) {
    ...
    */
    function pushState()
    {
        ...
        $_FLY_FLEXY_TOKEN['state']++;
        $s = $_FLY_FLEXY_TOKEN['state'];

        $_FLY_FLEXY_TOKEN['statevars'][$s] = array(); // initialize statevars
    }
    /**
    * tell the generator you are entering a block
    *
    ...
    */
    ...
    function findVar($string)
    {
        ...
        if (@$GLOBALS['_FLY_FLEXY']['currentOptions']['globals'] &&
            preg_match('/^(_POST|_GET|_REQUEST|_SESSION|_COOKIE|GLOBALS)\[/',$string)) {
            return '$'.$string;
        }
        if (!@$GLOBALS['_FLY_FLEXY']['currentOptions']['privates'] &&
                ($string{0} == '_')) {
                return Fly_Flexy::raiseError('Fly_Flexy::Attempt to access private variable:'.
                    " on line {$this->line} of {$GLOBALS['_FLY_FLEXY']['filename']}".
    ...
    */
    ...
    function pushVar($string)
    {
        ...
        global $_FLY_FLEXY_TOKEN;
        $s = $_FLY_FLEXY_TOKEN['state'];
        $_FLY_FLEXY_TOKEN['statevars'][$s][] = $string;
    }
    ...
    */
    ...
    function scopeVarsToArrayString()
    {
        ...
        $ar = array();
        for ($s = $_FLY_FLEXY_TOKEN['state']; $s > 0; $s--) {
            $ar = array_merge($ar, $_FLY_FLEXY_TOKEN['statevars'][$s]);
        }
        if (empty($ar)) {
            return 'array()';
        }
