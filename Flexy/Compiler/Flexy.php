...
class Fly_Flexy_Compiler_Flexy extends Fly_Flexy_Compiler {
    ...
    */
    function compile(&$flexy, $string=false)
    {
        ...
        // note this should be moved to new Fly_Flexy_Token
    ...
    function getModifierWrapper($element)
    {
        ...
        switch ($modifier) {
            ...
            case 'n':
                ...
                $suffix = $GLOBALS['_FLY_FLEXY']['currentOptions']['numberFormat'] . ')';
                break;
    ...
    */
    ...
    public function toStringMethodChain($element)
    {
        ...
        if (($object == 'GLOBALS') &&
            ...
            $var = $method;
        } elseif(is_array($element->args) && count($element->args) > 0
                && $element->args[0] instanceof Fly_Flexy_Token_MethodChain) {
            $chainedString = $this->toStringMethodChain(array_shift($element->args));
            if (count($bits)) {
                $var = $element->toVar('tmp.' . implode('.', $bits));
            } else {
                $var = $element->toVar('tmp');
            }
