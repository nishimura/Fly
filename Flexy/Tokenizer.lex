...
%%
%namespace Fly_Flexy_Tokenizer
%public
%class Fly_Flexy_Tokenizer
%implements yyParser.yyInput 
%type int
%ignore_token  FLY_FLEXY_TOKEN_NONE
%eofval{
	return TOKEN_EOF;
%eofval}
...
%full
%char
%state IN_SINGLEQUOTE IN_TAG IN_ATTR IN_ATTRVAL IN_NETDATA IN_ENDTAG IN_DOUBLEQUOTE IN_MD IN_COM IN_DS IN_FLEXYMETHOD IN_FLEXYMETHODQUOTED IN_FLEXYMETHODQUOTED_END IN_SCRIPT IN_CDATA IN_DSCOM IN_PHP IN_COMSTYLE IN_METHODCHAIN
...
<IN_METHODCHAIN>{FLEXY_METHOD}"(" {
    $this->flexyMethod = substr($this->yytext(),0,-1);
    $this->yybegin(IN_FLEXYMETHOD);
	return FLY_FLEXY_TOKEN_NONE;
}

<IN_METHODCHAIN>({FLEXY_VAR}":"{FLEXY_MODIFIER}"}")|({FLEXY_VAR}"}") {
    $t =  $this->yytext();
    $t = substr($t,1,-1);
